...
### Enable Asio (Optional)

1. Add `"asio"` to `vcpkg.json` dependencies
2. Add `-DBYTE_TORRENT_HAS_ASIO=ON` to CMake configure
3. Use `AsioSocket` for real TCP connections
...
int main() {
    ...
    auto client = CreateClientFromFile("ubuntu.torrent");
    ...
    // Main loop
    while (client->IsRunning() && !client->IsComplete()) {
        client->Update();
        
        auto stats = client->GetStats();
        PrintProgress(stats);  // Built-in progress display
...
class MyHandler : public ITorrentClientHandler {
    void OnStateChanged(ClientState state) override {
        std::println("State: {}", ToString(state));
    }
    
    void OnProgressUpdated(ClientStats const& stats) override {
        std::println("Progress: {:.1f}%", stats.progress * 100);
    }
    
    void OnPieceComplete(std::uint32_t piece) override {
        std::println("Piece {} verified", piece);
    }
    
    void OnDownloadComplete() override {
        std::println("Download finished!");
    }
    
    void OnError(std::string const& msg) override {
...
```cpp
// Add peers manually (e.g., from DHT or PEX)
client->AddPeer({.ip = "192.168.1.100", .port = 6881});

std::vector<PeerEndpoint> peers = GetPeersFromDHT();
...
// Transfer limits (0 = unlimited)
config.max_download_rate = 0;
config.max_upload_rate = 0;

// Timing
config.announce_interval = std::chrono::seconds{1800};
config.choke_interval = std::chrono::seconds{10};
config.keep_alive_interval = std::chrono::seconds{120};

// Piece selection
config.selection_strategy = SelectionStrategy::RarestFirst;
